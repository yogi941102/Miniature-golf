...
public class MainMenu: BaseMenuState 
{
	...
	public string achievementsSTR = "Achievements";
	/// <summary>
	...
	public override void onGUI()
	{
		...
		//draw the button
		if(GUI.Button (GUIHelper.screenRect (offsetX+.05f,offsetY+.275f,.35f,.1f), startButtonSTR))
		{
		...
		if(GUI.Button (GUIHelper.screenRect (offsetX+.05f,offsetY+.4f,.35f,.1f), optionsSTR))
		{
			swapObjects(optionsGO);
		}

		//draw the credits  button
		if(GUI.Button (GUIHelper.screenRect (offsetX+.05f,offsetY+.525f,.35f,.1f), creditsSTR))
		{
			swapObjects(creditsGO);
		}	

		/*
		if(GUI.Button (GUIHelper.screenRect (offsetX+.05f,offsetY+0.65f,.35f,.1f), achievementsSTR))
		{
			swapObjects(achievmentGO);
		}	*/
		if(useHighscoreButton)
		{
			if(GUI.Button (GUIHelper.screenRect (offsetX+.05f,offsetY+0.65f,.35f,.1f), highscoreSTR))
			{
		...
		if(useQuitButton)
		{
			if(GUI.Button (GUIHelper.screenRect (offsetX+.05f,offsetY+.775f,.35f,.1f),quitSTR))
			{
				Application.Quit();
			}				
		}
