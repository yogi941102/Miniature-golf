...
/// <summary>
/// the graphics menu state will set the quality of graphics
/// </summary>
public class GraphicsState : BaseMenuState 
{
	...
	public override void onGUI()
	{
		...
		GUI.Box( GUIHelper.screenRect (offsetX+0.525f,offsetY,.4f,.45f), "Graphics Menu");
		...
		if(GUI.Button(GUIHelper.screenRect (offsetX+0.6f,offsetY+0.2f,.1f,.1f),"+"))		
		{
			QualitySettings.IncreaseLevel();
		}
		
		//draw a button that will decrease the graphics quality
		if(GUI.Button(GUIHelper.screenRect (offsetX+0.75f,offsetY+0.2f,.1f,.1f),"-"))		
		{
